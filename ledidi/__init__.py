...
from .ledidi import Ledidi, TensorFlowRegressor
...
def fasta_to_ohe(
    ...
    for i, (name, sequence) in enumerate(sequences.items()):
        encodings[name] = sequence_to_ohe(
            sequence,
            ignore=ignore,
            alphabet=alphabet,
            dtype=dtype,
...
def bigwig_to_arrays(
    filename, include_chroms=None, exclude_chroms=None, fillna=0, dtype="float32"
):
